 ...
 */
export class NewIntegrationWizard {
    public static currentPanel: NewIntegrationWizard | undefined;
    ...
    public static render(extensionPath: string) {
        if (NewIntegrationWizard.currentPanel) {
            // If the webview panel already exists reveal it
            NewIntegrationWizard.currentPanel._panel.reveal(vscode.ViewColumn.One);
        } else {
            // If a webview panel does not already exist create and show a new one
            const panel = vscode.window.createWebviewPanel(
                "showHelloWorld",
                "New Integration Project Wizard",
                vscode.ViewColumn.One,
                {
                    enableScripts: true,
                }
            );
            NewIntegrationWizard.currentPanel = new NewIntegrationWizard(panel, extensionPath);
        }
    }
     ...
     */
    public dispose() {
        NewIntegrationWizard.currentPanel = undefined;
        ...
        return `
            ...
            <body>
                ...
                <script>
                    ...
                    const pageName = "NewIntegrationWizard";
                </script>
    ...
    private _setWebviewMessageListener(webview: vscode.Webview) {
        webview.onDidReceiveMessage(
            (message: any) => {
                ...
                switch (command) {
                    case "CreateNewIntegrationProject":
                        {
                            ArchetypeModule.createESBProject(message.groupId, message.artifactId);
                            this.dispose();
                            return;
                        }
                }
